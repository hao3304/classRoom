...
<!--        </div>-->
      ...
      <div class="login-form__right">
        ...
        <div>
          <Form ref="form" :model="form" :rules="rules">
            <FormItem prop="account">
              <Input v-model="form.account" size="large" placeholder="请输入企业/单位名称" >
                <Icon type="md-contact" slot="prefix" />
              </Input>
            </FormItem>
            <FormItem prop="password">
              <Input @on-enter="onSubmit" type="password" v-model="form.password" size="large" placeholder="请输入密码" >
        ...
        <p v-show="error" style="text-align: center;color: #f98989;margin-top: 5px;">{{error}}</p>
        <div class="btn-register">
          还没有注册帐号？<a  @click="mode = 'register'" href="javascript:;">注册帐号</a>
        </div>
      </div>
    </div>
    ...
    <div class="register-form" v-show="mode == 'register'" >
      ...
      <Form ref="register" :model="model" :rules="registerRules" style="padding: 0 80px">
        <Row :gutter="10">
          <!--<i-col :span="8">-->
          <!--<FormItem prop="type">-->
          <!--<Select  v-model="model.type" placeholder="请选择类型">-->
          <!--<Option value="S002" label="单位用户"></Option>-->
          <!--<Option value="S003" label="企业用户"></Option>-->
          <!--</Select>-->
          <!--</FormItem>-->
          <!--</i-col>-->
          <i-col :span="24">
            ...
            <FormItem prop="title">
              <Input v-model="model.title" placeholder="请输入企业/单位名称" />
            </FormItem>
          </i-col>
        </Row>


        <FormItem prop="registration">
          <Input v-model="model.registration" placeholder="请输入社会统一信用代码" />
        </FormItem>

        <FormItem prop="license">
          <Input  style="display: none" v-model="model.license"  />
          <Upload
                  multiple
                  type="drag"
                  ...
                  :action="action">
            <div style="padding: 10px 0" v-if="!model.license">
              <Icon type="ios-cloud-upload" size="52" style="color: #3399ff"></Icon>
              <p>请上传营业执照【扫描件】</p>
            </div>
            <div  class="license-img" v-else>
              <img :src="`${baseURL}/image/${model.license}/80/80`" alt="">
            </div>
          </Upload>
        </FormItem>
        ...
        <FormItem prop="passwdCheck">
          <Input type="password" v-model="model.passwdCheck" placeholder="请再次输入密码" />
        </FormItem>
        <FormItem>
          <Button type="info" size="large" @click="onRegister" long>注册</Button>
        </FormItem>
      </Form>

      <div class="btn-login">已有账号，<a @click="mode = 'login'" href="javascript:;">立即登录</a></div>
    </div>
...
<script>
  ...
  export default {
    ...
    data() {
      ...
      return {
        action: baseURL + "/biz/uploadLicense",
        baseURL,
        form: {
          account: "admin",
          password: ""
      ...
      onRegister() {
        this.$refs.register.validate(valid => {
          if (valid) {
            ...
            //                   title: "提示",
            //                   content:
            //                           "注册成功！登录以后请先完善《湖州市公共供水计划用水户基本情况登记表》。"
            //                 });
            //               }, 500);
            //             })
            //             .catch(() => {
