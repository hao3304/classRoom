        ...
        <Split slot="right" mode="vertical" v-model="vSplit">
          <f-panel slot="top" :header="false"  style="height: 100%">
            <div slot="body" slot-scope="param">
              <f-editor ref="editor" :code="code" :height="param.bodyHeight"></f-editor>
            </div>
          </f-panel>
          <f-panel slot="bottom" style="height: 100%">
            <div slot="header" class="p-problem__toolbar">
              ...
              <Button :loading="loading" style="margin-left: 20px" size="small" @click="onBuild" icon="md-play" type="success">编译</Button>
            </div>
            <div slot="body" slot-scope="params" id="terminal">
            </div>
          </f-panel>
        </Split>
      </Split>
...
`
    ...
    mounted() {
      ...
      this.socket = io("//" + window.location.host + '/ide');
      this.socket.close();
