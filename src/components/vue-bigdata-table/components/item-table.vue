<template>
  <div class="vue-bigdata-table-item-table">
    <table
      ...
      @paste="handlePaste"
      :class="[
        'vue-bigdata-table-data-table',
        'vue-bigdata-table-fixed-table',
        showFixedBoxShadow ? 'box-shadow' : ''
      ]"
      cellspacing="0"
      cellpadding="0"
      border="0"
    >
      ...
      <tbody>
        <tr
          ...
          :style="{
            background:
              currentMouseEnterIndex === index && canSelectText ? '#ebf7ff' : ''
          }"
          :class="[
            stripe && (indexBase + index) % 2 !== 0 ? 'stripe-gray' : '',
            tr.className,
            currentScrollToRowIndex === indexBase + index
              ? 'scroll-to-row-tip'
              : '',
            indexBase + index === highlightRowIndex ? 'highlight-row' : ''
          ]"
        ...
        >
          <td
            ...
            :class="[
              ...
              'vue-bigdata-table-data-table-center'
            ]"
          >
            <render-dom
              :render="indexRender"
          ...
          >
            <template
              v-if="
                !canEdit ||
                  (canEdit && `${indexBase + index}-${i}` !== edittingTd)
              "
            >
              <div
              ...
              <template v-else>
                <render-dom
                  ...
                  :back-value="{
                    row: indexBase + index,
                    col: i,
                    value: typeof td === 'object' && td !== null ? td.value : td
                  }"
                ></render-dom>
              </template>
            </template>
